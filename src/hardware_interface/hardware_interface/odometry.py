...
class Odometry(Node):
    def __init__(self):
        ...
        self.can_driver_subscriber = self.create_subscription(
            EncoderFeedback, '/encoder_feedback', self._encoder_feedback_callback, 10,
        ...
        self.joint_feedback_publisher = self.create_publisher(EncoderFeedback, '/gripper_feedback', 10)
    ...
    def _encoder_feedback_callback(self, msg: EncoderFeedback) -> None:
        with self._lock:
            s = self._state
            if msg.can_id == GRIPPER_MOTOR_ID:
                self._set_joint_params(msg, s)
                return
        ...
        with self._lock:
            ...
            if stationary:
                ...
                vx_local, vy_local = self._select_velocity_source(s)
        ...
        if result is not None:
            vx_g, vy_g, x, y, yaw_q = result
            self._publish_odometry(vx_g, vy_g, x, y, yaw_q)
            self._publish_odometry_local(vx_local, vy_local, yaw_q)

    def _classify_can_id(self, can_id: int) -> int | None:
    ...
    def _set_joint_params(self, msg: EncoderFeedback, s: OdomState) -> None:
        s.joint_position = msg.position
        s.joint_speed = msg.speed
        self._publish_joint(s.joint_position, s.joint_speed)

    def _select_velocity_source(self, s: OdomState):
    ...
    def _is_stationary_by_raw_speed(s: OdomState) -> bool:
        ...
        vx_local, vy_local = Odometry._forward_kinematics(s.motor_vel)
    ...
    def _integrate_position(
        self, s: OdomState,
        vx_global: float, vy_global: float,
        yaw: float, stationary: bool
    ) -> Tuple[float, float, float, float, Quaternion] | None:
    ...
    def _publish_odometry(self, vx: float, vy: float, x: float, y: float, yaw_q: Quaternion) -> None:
        ...
        msg.pose.pose.orientation = yaw_q
        self.current_odom_publisher.publish(msg)
        self.get_logger().info(f'Odom global: vel ({vx:.4f}, {vy:.4f}), pos ({x:.4f}, {y:.4f})')

    def _publish_odometry_local(self, vx_local: float, vy_local: float, yaw_q: Quaternion) -> None:
        msg = OdomMsg()
        msg.twist.twist.linear.x = vx_local
        msg.twist.twist.linear.y = vy_local
        msg.pose.pose.orientation = yaw_q 
        self.local_odom_publisher.publish(msg)
        self.get_logger().debug(f'Odom local: vel ({vx_local:.4f}, {vy_local:.4f})')
    ...
    def _publish_joint(self, joint_position: float, joint_speed: float) -> None:
        ...
        self.get_logger().debug(f'Gripper joint: position={joint_position:.4f}, speed={joint_speed:.4f}')
    ...
    def _unit_conversion(s: OdomState, idx: int, msg: EncoderFeedback) -> float:
        ...
        return WHEEL_RADIUS_M * s.motor_position[idx]
