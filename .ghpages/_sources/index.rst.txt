Target Setter Documentation
===========================

Welcome to the official documentation for **Target Setter**.
This documentation provides system overview, installation instructions,
technical references, and theoretical background used in the project.

Core Documentation
------------------

.. toctree::
   :maxdepth: 2

   introduction
   hardware_specification
   installation_and_configuration
   technical_reference
   troubleshooting_guide


Appendices (Technical Background)
---------------------------------

.. toctree::
   :maxdepth: 1

   appendix_a_object_oriented_programming
   appendix_b_mvc_architecture
   appendix_c_mvvm_architecture
   appendix_d_ros2_overview
   appendix_e_distributed_systems
   appendix_f_monolithic_architecture
   appendix_g_ros2_architecture
   appendix_h_udp
   appendix_i_quaternions
   appendix_j_kinematics
   appendix_k_state_machine
   appendix_l_pid_control
   appendix_m_localization
   appendix_n_odometry
   appendix_o_waypoint_navigation
   appendix_p_complementary_filtering
   appendix_q_can_bus

Indices and Tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`

.. bibliography::
   :style: plain
   :labelprefix: [Ref]
   :keyprefix: ref-
   :all: